...
configure_make() {
  ...
  sed -i 's/using gcc ;/using gcc : android : '"${TOOL}"'-g++ ;/' project-config.jam
  ...
  OUTPUT_ROOT=${TOOLS_ROOT}/../output/boost/${ABI}
...
OUTPUT_INCLUDE=${TOOLS_ROOT}/../output/boost/include
[ -d ${OUTPUT_INCLUDE} ] || mkdir -p ${OUTPUT_INCLUDE}
cp -r ${LIB_DEST_DIR}/${OUTPUT_ABI}/include/boost ${OUTPUT_INCLUDE}
